...
const dataFilter = computed(() => {
  return props.dataBarang.filter(p => p.status === 'disetujui' || p.status === 'disetujui_jurusan');
});
</script>
...
<template>
  <section class="bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden">
    <div class="p-5 border-b border-slate-100 bg-slate-50/50">
      <h2 class="text-lg font-bold text-emerald-700">Arsip Persetujuan Peminjaman Alat</h2>
      <p class="text-xs text-slate-400 mt-0.5">Berkas peminjaman sarana prasarana siswa yang telah sukses divalidasi.</p>
    </div>
    ...
    <div v-else-if="dataFilter.length === 0" class="py-12 text-center text-slate-400">
      ...
      <p class="text-sm font-bold">Belum ada arsip peminjaman disetujui.</p>
    </div>
        ...
        <tbody class="divide-y divide-slate-100 text-sm text-slate-700">
          <tr v-for="(p, i) in dataFilter" :key="p.id" class="hover:bg-slate-50/50">
            ...
            <td class="px-6 py-4 text-center">
              <span class="px-3 py-1 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-full text-xxs font-bold uppercase">● {{ p.status }}</span>
            </td>
          </tr>
        </tbody>
      </table>
