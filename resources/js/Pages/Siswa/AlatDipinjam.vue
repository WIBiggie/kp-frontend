...
import { Search, Info, Calendar } from 'lucide-vue-next';
...
// Memfilter riwayat yang statusnya masih aktif (Dipinjam atau Terlambat)
const aktifDipinjam = computed(() => {
  const listArr = props.daftarRiwayat || [];

  const listAktif = listArr.filter(item => {
    const status = item.status ? item.status.toLowerCase() : '';
    return status === 'dipinjam' || status === 'terlambat';
  });

  return listAktif.filter(item => {
  ...
  <div class="space-y-6">
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-4">
      <div class="relative w-full">
        ...
        <input v-model="searchQuery" type="text" placeholder="Cari nama atau kode alat yang sedang dipinjam..." class="w-full pl-11 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-indigo-500 transition" />
      </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
      <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="bg-amber-500 text-xxs font-bold uppercase tracking-wider text-white">
              ...
              <th class="py-4 px-6 text-center w-40">Status Alat</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-slate-50">
            <tr v-if="aktifDipinjam.length === 0">
              <td colspan="6" class="py-10 text-center text-slate-400 text-sm">
                <Info :size="24" class="mx-auto mb-2 text-slate-300" />
                Tidak ada alat yang sedang Anda pinjam saat ini.
              </td>
            </tr>
            <tr v-for="(log, index) in aktifDipinjam" :key="log.id" class="hover:bg-slate-50/50 transition">
              ...
              <td class="py-4 px-6">
                <div class="flex items-center gap-1.5 text-xs font-medium text-slate-600">
                  <Calendar :size="12" class="text-amber-500" /> {{ log.tanggal_pinjam || log.tgl_pinjam }}
                </div>
              </td>
              <td class="py-4 px-6 text-center">
                <span v-if="log.status && log.status.toLowerCase() === 'terlambat'" class="px-2.5 py-1 rounded-full text-xs font-bold bg-rose-50 text-rose-700 border border-rose-100">
                  ● Terlambat
                </span>
